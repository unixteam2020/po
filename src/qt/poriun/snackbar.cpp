...
#include "qt/poriun/forms/ui_snackbar.h"
#include "qt/poriun/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PoriunGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PoriunGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
