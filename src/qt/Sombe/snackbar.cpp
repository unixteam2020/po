...
#include "qt/Sombe/forms/ui_snackbar.h"
#include "qt/Sombe/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SombeGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SombeGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
