...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BPR);
    ...
    if (settings.contains("nAnonymizePoriunAmount") || reset)
        SoftSetArg("-anonymizeporiunamount", settings.value("nAnonymizePoriunAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in poriun.cpp during shotdown to also remove the window positions
