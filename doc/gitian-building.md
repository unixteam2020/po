...
*Setup instructions for a gitian build of Sombe Coin using a VM or physical system.*
...
to the Sombe GitHub release page.
...
- [Building Sombe Coin](#building-Sombe-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/Sombe-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/Sombe-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/Sombe-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/Sombe-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/Sombe-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/Sombe-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/Sombe-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@Sombe.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/Sombe-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/Sombe-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/Sombe-osx-*-build.assert
```
