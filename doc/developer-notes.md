...
- [Development guidelines](#development-guidelines)
    - [General Sombe Coin](#general-Sombe-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Sombe Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Sombe Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Sombe-project/Sombe/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_Sombe
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_Sombe --log_level=test_suite
$ valgrind -v --leak-check=full src/Sombed -printtoconsole
```
...
# A coverage report will now be accessible at `./test_Sombe.coverage/index.html`.
```
...
However, in open source software such as Sombe Coin, where everyone uses
...
pay attention to for reviewers of Sombe Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Sombe Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Sombe Coin so that they can be integrated
...
file descriptors it will cause problems with Sombe's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `Sombe-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `Sombe-cli` and the GUI debug console use this table to determine how to
