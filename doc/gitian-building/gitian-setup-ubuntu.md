...
The build script we'll be using is contained in the Sombe github repository ([contrib/gitian-build.py](https://github.com/Sombe-project/Sombe/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Sombe repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Sombe-Project/Sombe/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, Sombe-detached-sigs, and Sombe GitHub repos.
