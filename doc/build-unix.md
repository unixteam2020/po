UNIX BUILD NOTES
====================
Some notes on how to build Poriun Coin in Unix.

Note
---------------------
Always use absolute paths to configure and compile Poriun Coin and the dependencies,
...
This will build poriun-qt as well, if the dependencies are met.
...
memory available when compiling Poriun Coin. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~poriun/+archive/poriun).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:poriun/poriun
    sudo apt-get update
...
To build Poriun Coin without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build poriun-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a poriun-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip poriund" to strip the debug
...
Security
--------
To help make your Poriun Coin installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, Poriun Coin should be built with a non-executable stack
